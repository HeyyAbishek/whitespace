...
export default function Canvas() {
  ...
  const undo = useCallback(() => {
    try { historyUndo(); } catch (error) { console.error("Undo failed:", error); }
  }, [historyUndo]);
  ...
  const addElement = useMutation(({ storage }, newShape: Layer) => {
    try {
      ...
      if (!existingElements) return false;
      
      if (existingElements.length >= MAX_ELEMENTS) {
        alert('Board is full! Please delete some elements.');
        return false;
      }
      
      const newSize = getLayerSize(newShape);
      
      if (currentStorageSize.current + newSize > MAX_STORAGE_SIZE) {
        alert('Board storage is full! Please clear some space.');
        return false;
      }
    ...
    if (node) {
        ...
        toPng(node, { filter, backgroundColor: isDarkMode ? '#121212' : '#f8f9fa' })
          ...
          .catch(error => {
            ...
            alert('Failed to export. Try clearing some elements.');
          });
    }
  }, [isDarkMode]);
  ...
  const handlePointerUp = useCallback(() => {
    ...
    if (drawingId) {
      ...
      if (el?.type === 'pencil' && el.points && el.points.length > 5) {
        ...
        updateElement({ id: drawingId, updates: { points: simplified } });
      }
    }
